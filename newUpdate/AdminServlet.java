		...
		try {
			...
			if(arr.length != 4) {
				...
				if(arr[3].equals("dashboard")) {
					...
					res = ResponseUtil.buildResponse(AdminManager.getAnnouncements(), "data recived");
				}
					...
					res=ResponseUtil.buildResponse(AdminManager.getAnnouncement(id), "data recived");
				}
					...
					LOGGER.info(new StringBuilder("::: Get data for userdetails to convert the csv file ::: querying into AdminManager :::").toString());
				 	...
				 	ResponseUtil.processResponseFile(response, ResponseUtil.buildResponse(AdminManager.getUsers(), "data recived"));
					return;
				}else if(arr[3].equals("contentType")) {
					LOGGER.info(new StringBuilder("::: Get data for questions ::: querying into AdminManager :::").toString());
					
					res = ResponseUtil.buildResponse(AdminManager.getContentType() , "data recived");
				...
				else if(arr[3].equals("question")) {

				    int id = Integer.parseInt(request.getParameter("id"));

				    res = ResponseUtil.buildResponse(AdminManager.getQuestionById(id),"question fetched");
				}
				else { 
					LOGGER.error(new StringBuilder("::: Bad request ::: No url found   :::").toString());
		...
		try {
			...
			if(arr.length != 4) {
				...
				if(arr[3].equals("questions")) {
					...
					AdminManager.addMessage(title, message, clientId);
				...
				else if(arr[3].equals("editQuestion")) {

				    res = ResponseUtil.buildResponse(
				        new JSONObject().put("result", AdminManager.updateQuestion(payload)),
				        "question updated"
				    );
				}
				else { 
