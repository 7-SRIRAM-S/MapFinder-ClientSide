...
public class AnnouncementServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		responseObject = ResponseUtil.buildResponse(announcements, message);
		...
		try {
			...
			if (ValidationUtil.isNotEmpty(title) && ValidationUtil.isNotEmpty(msg) && createdBy >= 0) {
				...
				if (isAnnouncementAdded) {
					...
					responce = ResponseUtil.buildResponse(obj, msg);
				} else {
					message = "Announcement didn't added";
					responce = ResponseUtil.buildErrorResponse(406, message);
			...
			message = "input invalid empty data found";
			responce = ResponseUtil.buildErrorResponse(500, message);
		}
